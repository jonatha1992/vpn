...
function setup() {
  const listeners = {};
  const actionCalls = {badge:[],color:[],title:[]};
  const state = {levelOfControl:'controlled_by_this_extension',value:{rules:{singleProxy:{host:'proxy.example',port:8080}}}};
  const chrome = {
    proxy:{settings:{get:async()=>state,onChange:{addListener:fn=>{listeners.onProxyChange=fn;}}},onProxyError:{addListener:fn=>{listeners.onProxyError=fn;}}},
    action:{
      setBadgeText:async value=>{actionCalls.badge.push(value);},
      setBadgeBackgroundColor:async value=>{actionCalls.color.push(value);},
  ...
  return {invoke,state,listeners,actionCalls};
}
test('shows the proxy state on the toolbar icon',async()=>{
  const {state,listeners,actionCalls}=setup();
  await new Promise(resolve=>setImmediate(resolve));
  assert.equal(actionCalls.badge.at(-1).text,'ON');
  assert.equal(actionCalls.title.at(-1).title,'Pasaporte VPN: activa');
  state.levelOfControl='controllable_by_this_extension';
  listeners.onProxyChange();
  await new Promise(resolve=>setImmediate(resolve));
  assert.equal(actionCalls.badge.at(-1).text,'OFF');
  assert.equal(actionCalls.title.at(-1).title,'Pasaporte VPN: desactivada');
});
